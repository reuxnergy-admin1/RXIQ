...
class TestExtractMocked:
    ...
    @patch(
        "app.routes.v1.cache.get_cached",
        new_callable=AsyncMock,
        return_value=MOCK_CONTENT.model_dump(),
    )
    ...
    def test_extract_connection_error_returns_502(self, mock_fetch, mock_get):
        response = client.post(
            "/api/v1/extract", json={"url": "https://unreachable.com"}
        )
    ...
    @patch(
        "app.routes.v1.scraper.fetch_html",
        new_callable=AsyncMock,
        side_effect=URLValidationError("Blocked host"),
    )
    ...
    @patch(
        "app.routes.v1.scraper.fetch_html",
        new_callable=AsyncMock,
        side_effect=ValueError("No text content"),
    )
...
class TestSummarizeMocked:
    ...
    def test_summarize_url_success(
        self, mock_fetch, mock_extract, mock_ai, mock_set, mock_get
    ):
        response = client.post(
            "/api/v1/summarize",
            json={"url": "https://example.com/article", "format": "bullets"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    @patch(
        "app.routes.v1.cache.get_cached",
        new_callable=AsyncMock,
        return_value=MOCK_SUMMARY.model_dump(),
    )
    ...
    def test_summarize_no_content_returns_422(
        self, mock_fetch, mock_extract, mock_set, mock_get
    ):
        empty = ExtractedContent(
            url="https://example.com", text="", word_count=0, excerpt=""
        )
...
class TestSentimentMocked:
    ...
    def test_sentiment_url_success(
        self, mock_fetch, mock_extract, mock_ai, mock_set, mock_get
    ):
    ...
    @patch(
        "app.routes.v1.cache.get_cached",
        new_callable=AsyncMock,
        return_value=MOCK_SENTIMENT.model_dump(),
    )
    ...
    def test_sentiment_no_content_returns_422(
        self, mock_fetch, mock_extract, mock_set, mock_get
    ):
        empty = ExtractedContent(
            url="https://example.com", text="  ", word_count=0, excerpt=""
...
class TestSEOMocked:
    ...
    @patch(
        "app.routes.v1.cache.get_cached",
        new_callable=AsyncMock,
        return_value=MOCK_SEO.model_dump(),
    )
...
class TestAnalyzeMocked:
    ...
    @patch(
        ...
        new_callable=AsyncMock,
        return_value=MOCK_KEYWORDS_RESULT,
    )
    @patch(
        "app.routes.v1.ai_service.analyze_sentiment",
        new_callable=AsyncMock,
    ...
    def test_analyze_no_content_returns_422(
        self, mock_fetch, mock_extract, mock_seo, mock_set, mock_get
    ):
        empty = ExtractedContent(
            url="https://example.com", text="", word_count=0, excerpt=""
        )
    ...
    @patch(
        "app.routes.v1.scraper.fetch_html",
        new_callable=AsyncMock,
        side_effect=URLValidationError("Blocked"),
    )
...
class TestHandleScrapeError:
    ...
    def test_http_status_error(self):
        ...
        exc = _handle_scrape_error(
            httpx.HTTPStatusError("err", request=resp.request, response=resp), "test"
        )
